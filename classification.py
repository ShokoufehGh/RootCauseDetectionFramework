...
def run_pattern(csv_url, input_data):
    ...
    for i, config in enumerate(configurations):
        ...
        if selected_part == "p1":
            ...
            df, split_dfs = p2(config, synonyms, csv_url, NULL, period)
            ...
            df, split_dfs = p3(synonyms[0], config[0], config[1], synonyms, csv_url, NULL, shuffle)
            ...
            df, split_dfs = p4(synonyms[0], config[0], config[1], synonyms, csv_url, NULL)
            ...
            minimum_day_activity = 5
            df, split_dfs = p6(synonyms[0], config[0], config[1], synonyms, csv_url, NULL, minimum_day_activity, shuffle)
                
        frequencies = [len(split_df) for split_df in split_dfs]
...
def main():
    log_file = "log/classification_22Apr_p4(1)_overall_workload_tree_with_workload_with_new_log.txt"    
