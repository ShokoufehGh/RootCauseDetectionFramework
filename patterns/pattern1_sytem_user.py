...
def print_percentage_of_users(df, target_percentage):
    ...
    # Group by 'event User' and count the number of events for each user
    user_event_counts = df['event User'].value_counts()
    
    # Calculate and print the percentage of events for each user
    ...
    best_combination = []
    best_sum = 0
    min_diff = float('inf')  # Track the smallest difference from the target
    
    # Iterate through all possible combinations of users
    ...
    # Print the best combination and their total percentage
